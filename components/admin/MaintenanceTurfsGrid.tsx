...
export default function MaintenanceTurfsGrid({ searchQuery, onRefresh }: MaintenanceTurfsGridProps) {
  ...
  async function fetchMaintenanceVenues() {
    try {
      setLoading(true);
      const data: any = await adminApi.getMaintenanceTurfs({ page: 1, limit: 100, search: searchQuery });
  ...
  async function toggleMaintenance(venueId: string, currentStatus: boolean) {
    try {
      await adminApi.updateVenue(venueId, { is_active: !currentStatus });
  ...
  const getSportIcon = (sport: string) => {
    const icons: Record<string, string> = {
      football: 'sports_soccer', cricket: 'sports_cricket',
      badminton: 'sports_tennis', multi: 'sports_kabaddi',
    };
    return icons[sport] || 'sports';
  };

  if (loading) return <LoadingSpinner size="lg" text="Loading maintenance venues..." />;
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
      {venues.map((venue) => (
        <div key={venue.id} className="group bg-white/5 border border-primary/5 rounded-xl overflow-hidden hover:border-primary/40 transition-all duration-300">
          <div className="relative h-48 overflow-hidden">
            ...
            <div className="absolute top-4 left-4">
              <span className="flex items-center gap-1 bg-gray-500 text-white text-[10px] font-bold px-2 py-1 rounded-full">INACTIVE</span>
            </div>
            <div className="absolute top-4 right-4">
              <a href={`/admin/venues/edit/${venue.id}`} className="w-8 h-8 bg-black/50 backdrop-blur-md text-white rounded-lg flex items-center justify-center hover:bg-primary hover:text-black transition-colors">
          ...
          <div className="p-5">
            {venue.turfs && venue.turfs.length > 0 && (
              <div className="mb-4 pb-4 border-b border-primary/10">
                <p className="text-[10px] uppercase tracking-wider text-gray-400 font-semibold mb-2">Available Sports</p>
                <div className="flex flex-wrap gap-2">
                  {venue.available_sports.slice(0, 3).map((sport, i) => (
                    <span key={i} className="inline-flex items-center gap-1 px-2 py-1 bg-gray-500/10 text-gray-300 text-xs rounded-full border border-gray-500/20">
                      <span className="material-symbols-outlined text-xs">{getSportIcon(sport)}</span>
                ...
                {venue.min_price > 0 && (
                  <div className="mt-2 text-xs text-gray-400">
                    ...
                    {venue.min_price !== venue.max_price && <> - <span className="text-white font-bold">₹{venue.max_price}</span></>}/hr
                  </div>
                )}
              </div>
            )}
            ...
            <div className="flex items-center justify-between mb-4 pb-4 border-b border-primary/10">
              <div>
                <p className="text-[10px] uppercase tracking-wider text-gray-400 font-semibold mb-1">Owner</p>
                <div className="flex items-center gap-2">
                  <div className="w-6 h-6 rounded-full bg-primary/20 flex items-center justify-center text-primary text-xs font-bold">
                    {getInitials(venue.owner?.full_name || 'U')}
                  </div>
                  <span className="text-sm font-medium text-white">{venue.owner?.full_name || 'Unknown'}</span>
                </div>
              </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <label className="relative inline-flex items-center cursor-pointer">
                ...
                />
                <div className="w-11 h-6 bg-gray-800 rounded-full peer peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-primary"></div>
              </label>
            </div>
          </div>
        </div>
